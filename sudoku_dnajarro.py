...
class Sudoku():
    ...
    def forward_check(self, row, column, value, mode='remove'):
        ...
        if mode == 'remove':
            # check the variables in the same column
            for i in range(9):
                if i != row:
                    if self.cells[i][column].remove_value(value) == False:
                        return False
            # check the variables in the same row
            for j in range(9):
                if j != column:
                    if self.cells[row][j].remove_value(value) == False:
                        return False
            gridnum, cell = self.get_grid_cell(row, column)
            gridlocations = []
            # check the variables in the same 3x3 grid
            for x in range(9):
                gridrow, gridcol = self.get_row_column(gridnum, x)
                gridlocations.append((gridrow, gridcol))
            for cell in gridlocations:
                if cell[0] != row and cell[1] != column:
                    if self.cells[cell[0]][cell[1]].remove_value(value) == False:
                        return False
            return True
        elif mode == 'count':
            count = 0
            ...
            for j in range(9):
                if j != column:
                    if value in self.cells[row][j].domain:
                        count += 1
            gridnum, cell = self.get_grid_cell(row, column)
            gridlocations = []
            # check the variables in the same 3x3 grid
            ...
            for cell in gridlocations:
                if cell[0] != row and cell[1] != column:
                    if value in self.cells[cell[0]][cell[1]].domain:
                        count += 1
            return count
...
def mrv(puzzle, unassigned):
    ...
    # create dictionary of (size of domain -> values
    mrv_dict = { 1: [],
                2: [],
                 3: [],
                 4: [],
    ...
    for locationtuple in unassigned:
        row = locationtuple[0]
        col = locationtuple[1]
        for val in mrv_dict:
            if puzzle.cells[row][col].value == None and len(puzzle.cells[row][col].domain) == val:
                mrv_dict[val].append(locationtuple)
    for i in range(1, 10):
        if len(mrv_dict[i]) > 0:
            return mrv_dict[i]
...
def count_constraints(puzzle, row, column):
    ...
    count = 0
    # count the number of variables that are unassigned in the same column
    for col in range(9):
        if puzzle.cells[row][col].value == None:
            count += 1
    # count the number of variables that are unassigned in the same row
    for row in range(9):
        if puzzle.cells[row][column].value == None:
            count += 1
    gridnum, cell = puzzle.get_grid_cell(row, column)
    gridlocations = []
    # count the number of variables that are unassigned in the same 3x3 grid
    for x in range(9):
        gridrow, gridcol = puzzle.get_row_column(gridnum, x)
        gridlocations.append((gridrow, gridcol))
    for cell in gridlocations:
        if cell[0] != row and cell[1] != column:
            if puzzle.cells[cell[0]][cell[1]].value == None:
                count += 1
    return count
...
def order_values(puzzle, row, column):
    ...
    for val in domain:
        val_dict[val] = puzzle.forward_check(row, column, val, 'count')
    # sort the dictionary by value (i.e., the number of constraints in ascending order)
    val_dict = {k: v for k, v in sorted(val_dict.items(), key=lambda item: item[1])}
    return [key for key in val_dict]
...
def backtracking_search(puzzle):
    ...
    next_var_tuple = select_variable(puzzle)
    ...
    ordered_vals = order_values(puzzle, next_var_tuple[0], next_var_tuple[1])
    ...
    for val in ordered_vals:
        ...
        new_puzzle.cells[next_var_tuple[0]][next_var_tuple[1]].assign_value(val)
        ...
        isValid = new_puzzle.forward_check(next_var_tuple[0], next_var_tuple[1], val, 'remove')
